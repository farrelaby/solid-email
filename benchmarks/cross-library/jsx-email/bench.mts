/**
 * JSX-Email benchmark runner
 *
 * Modes:
 *   1. render – async render of static template via jsx-email
 *
...
import { createJsxMarketingEmail } from './template';
...
async function main() {
  ...
  const testHtml = await render(createJsxMarketingEmail(marketingProps));
  assertIncludes('jsx-email', testHtml, ['Launch Week', 'Product highlights']);

  mkdirSync(RESULTS_DIR, { recursive: true });
  writeFileSync(join(RESULTS_DIR, 'jsx-email.html'), testHtml, 'utf-8');
  ...
  const renderPerf = await benchAsync(
    'jsx-email render',
    async () => render(createJsxMarketingEmail(marketingProps)),
    WARMUP,
    RUNS,
    ITERATIONS_PER_RUN,
  );
